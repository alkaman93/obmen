...
def generate_top_15():
    names = ["Alice", "Bob", "Charlie", "David", "Eve", "Frank", "Grace", "Henry",
             "Ivy", "Jack", "Kate", "Leo", "Mia", "Nick", "Olivia", "Paul", "Quinn",
             "Rita", "Sam", "Tina"]
    random_top = []
    for _ in range(15):
...
def handle_message(message):
    ...
    if user_id in user_states:
        ...
        if state == 'waiting_username':
            ...
            if second_user:
                ...
                send_message(chat_id, "<b>⚠️ Введите корректный username!</b>")
            return
        ...
        if state == 'waiting_amount':
            try:
                ...
                if amount < settings['min_amount']:
                    send_message(chat_id, f"<b>⚠️ Минимальная сумма: ${settings['min_amount']}!</b>")
                    return
                if amount > settings['max_amount']:
                    send_message(chat_id, f"<b>⚠️ Максимальная сумма: ${settings['max_amount']}!</b>")
                    return
                ...
                send_message(chat_id, "<b>⚠️ Введите число!</b>")
            return
        ...
        if state == 'admin_limits' and user_id == ADMIN_ID:
            ...
            try:
                ...
                if len(parts) == 2:
                    ...
                    max_val = int(parts[1])
                    if min_val >= max_val:
                        send_message(chat_id, "<b>⚠️ Минимум должен быть меньше максимума!</b>")
                        return
                    settings['min_amount'] = min_val
                    settings['max_amount'] = max_val
                    send_message(chat_id, f"<b>✅ Лимиты обновлены: ${min_val} - ${max_val}</b>")
                else:
                    send_message(chat_id, "<b>⚠️ Формат: мин-макс (например: 100-500)</b>")
            except:
                send_message(chat_id, "<b>⚠️ Ошибка. Формат: 100-500</b>")
            return
    ...
    if text == "ℹ️ Информация":
        info_text = (
            "<b>🌟 Наш проект создан для безопасного обмена NFT подарками среди пользователей Telegram'а.</b>\n\n"
    ...
    if text == "📖 Как происходит сделка":
        deal_text = (
            ...
            f"• <b>Один участник создаёт сделку через меню бота - @{BOT_USERNAME} 📝</b>\n"
...
def handle_callback(callback):
    ...
    if data.startswith('accept_'):
        ...
        deal = deals[deal_id]
        if deal['status'] != 'waiting':
            edit_message(chat_id, message_id, "<b>⚠️ Сделка уже недоступна!</b>")
            return
        if user_id == deal['creator_id']:
            edit_message(chat_id, message_id, "<b>⚠️ Нельзя принять свою сделку!</b>")
            return
        if username.lower() != deal['second_user'].lower():
            edit_message(chat_id, message_id, "<b>⚠️ Эта сделка создана не для вас!</b>")
            return
    ...
    if data.startswith('cancel_'):
        ...
        if deal_id in deals:
            if deals[deal_id]['creator_id'] == user_id:
                ...
                edit_message(chat_id, message_id, "<b>⚠️ Только создатель может отменить сделку!</b>")
        return
